class Game:
    ...
    def complete_turn(self):
        for p_num in self.game_state['players']:
            initial_coordinates = self.game_state['players'][p_num]['scout_coords']
            choices = self.get_in_bounds_translations(initial_coordinates)

            for player in self.players:
                if player.player_number == p_num:
                    move = player.choose_translation(self.game_state, choices)

            self.game_state['players'][p_num]['scout_coords'] = (initial_coordinates[0] + move[0], initial_coordinates[1] + move[1])
